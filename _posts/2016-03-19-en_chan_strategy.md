---
title: Chan Theory trend strategy.
layout: post
category: ta
language: en
---

The example includes using Chan strategy to recognize trends and predicting the trend change percentage fitted by
ARIMA+GARCH statistical model(calling R software in python) which are applied on the trends recognized by Chan theory,
the code to perform backtesting of the strategy, and the code to investigate trades(switch between trend following and
trend reversion based on the prediction results) through generated charts.
